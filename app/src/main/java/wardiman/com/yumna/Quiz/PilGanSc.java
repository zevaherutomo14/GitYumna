...
public class PilGanSc extends AppCompatActivity {
    ...
    private void setonten() {
        ...
        if (x >= arr){
            ...
            Intent intent = new Intent(PilGanSc.this,SkorScActivity.class);
            intent.putExtra("skorAkhir", jumlahSkor);
            intent.putExtra("activity", "PilihanGanda");
            startActivity(intent);
        } else {
            mtvSoal.setText(soalPG.getBersuci(x));
            mrbJawaban1.setText(soalPG.getPilJwbSc1(x));
            mrbJawaban2.setText(soalPG.getPilJwbSc2(x));
            mrbJawaban3.setText(soalPG.getPilJwbSc3(x));
            jawaban = soalPG.getJwbBnrBersuci(x);
        }
        x++;
    }
    ...
    private void cekJawaban() {
        if (mrbJawaban1.isChecked()){
            if (mrbJawaban1.getText().toString().equals(jawaban)){
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(PilGanSc.this);

...
public class PilGanShl extends AppCompatActivity {
...
//        pemilihan bila soal sudah selesai dikerjakan dan sedang dikerjakan
        ...
        if (x >= arr){
            ...
            Intent intent = new Intent(PilGanShl.this,SkorShlActivity.class);
            intent.putExtra("skorAkhir", jumlahSkor);
            intent.putExtra("activity", "PilihanGanda");
            startActivity(intent);
        } else {
            mtvSoal.setText(soalPG.getShalat(x));
            mrbJawaban1.setText(soalPG.getPilJwbSh1(x));
            mrbJawaban2.setText(soalPG.getPilJwbSh2(x));
            mrbJawaban3.setText(soalPG.getPilJwbSh3(x));
            jawaban = soalPG.getJwbBnrShalat(x);
        }
        x++;
    }
    ...
    private void cekJawaban() {
        if (mrbJawaban1.isChecked()){
            if (mrbJawaban1.getText().toString().equals(jawaban)){
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(PilGanShl.this);
